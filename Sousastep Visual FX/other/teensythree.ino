...
void loop() {
  ...
  static uint8_t serial_array_index = 0;
  ...
  while (Serial.available() > 0) {
    ...
    if (serial_data == startMarker) {
      // found start marker, reset index and start collecting data
      startMarkerFound = true;
      serial_array_index = 0;
    } else if (serial_data == endMarker) {
      // found end marker, process the collected data
      startMarkerFound = false;
      if (serial_array_index == numChannels) {
      ...
      // collect data
      if (serial_array_index < numChannels) {
        serial_array[serial_array_index] = serial_data;
        serial_array_index++;
      } else {
        // too much data, discard it
      }
    }
  }
/*
